...
function StoreFormModal({ store, onClose, onSave }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      fd.append('schedules', JSON.stringify(form.schedules))
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', background: 'rgba(15,23,42,0.3)', backdropFilter: 'blur(4px)' }}>
      <div style={{ background: '#ffffff', border: '1px solid #f1f5f9', borderRadius: '20px', width: '100%', maxWidth: '720px', maxHeight: '95vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.15)', animation: 'db-modal-in 0.2s ease' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
          <div style={{ flex: 1, overflowY: 'auto', padding: '22px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            {section === 'hours' && (
              <>
                <SectionHeader icon={Clock} title="Opening Hours" subtitle="Set day-wise open, close, and break times" />
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {Object.keys(DEFAULT_SCHEDULES).map((day) => {
                    const sched = form.schedules[day] || DEFAULT_SCHEDULES[day];
                    const isClosed = !sched.isOpen;
                    return (
                      <div key={day} style={{ display: 'flex', flexDirection: 'column', gap: '10px', padding: '16px', border: '1.5px solid #e2e8f0', borderRadius: '12px', background: isClosed ? '#f8fafc' : '#ffffff' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          <span style={{ fontSize: '14px', fontWeight: '700', color: '#1e293b', textTransform: 'capitalize' }}>{day}</span>
                          <button
                            type="button"
                            onClick={() => set('schedules', { ...form.schedules, [day]: { ...sched, isOpen: !sched.isOpen } })}
                            style={{
                              display: 'flex', alignItems: 'center', gap: '6px',
                              padding: '6px 14px',
                              borderRadius: '20px',
                              border: 'none',
                              background: sched.isOpen ? '#dcfce7' : '#f1f5f9',
                              color: sched.isOpen ? '#15803d' : '#475569',
                              fontSize: '12px', fontWeight: '700',
                              cursor: 'pointer', transition: 'all 0.15s',
                            }}>
                        ...
                        {sched.isOpen && (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', borderTop: '1px solid #f1f5f9', paddingTop: '12px' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                              <span style={{ fontSize: '10px', fontWeight: '800', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Store Hours</span>
                              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                <Input label="Open" type="time" value={sched.open} style={{ cursor: 'pointer' }} onChange={e => set('schedules', { ...form.schedules, [day]: { ...sched, open: e.target.value } })} />
                                <Input label="Close" type="time" value={sched.close} style={{ cursor: 'pointer' }} onChange={e => set('schedules', { ...form.schedules, [day]: { ...sched, close: e.target.value } })} />
                              </div>
                            </div>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                              <span style={{ fontSize: '10px', fontWeight: '800', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Break Time</span>
                              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                <Input label="Start" type="time" value={sched.breakStart} style={{ cursor: 'pointer' }} onChange={e => set('schedules', { ...form.schedules, [day]: { ...sched, breakStart: e.target.value } })} />
                                <Input label="End" type="time" value={sched.breakEnd} style={{ cursor: 'pointer' }} onChange={e => set('schedules', { ...form.schedules, [day]: { ...sched, breakEnd: e.target.value } })} />
                              </div>
                            </div>
                          </div>
                        )}
                ...
                {(() => {
                  ...
                  const isStoreCurrentlyOpen = (schedules) => {
                    if (!schedules) return false;
                    const days = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'];
                    const now = new Date();
                    const dayName = days[now.getDay()];
                    const todaySched = schedules[dayName];
                    ...
                    const openMin = timeToMin(todaySched.open);
                    let closeMin = timeToMin(todaySched.close);
                    ...
                    let isOpenNow = false;
                    const adjNowMin = (closeMin >= 24 * 60 && nowMin < openMin) ? nowMin + 24 * 60 : nowMin;
                    
                    if (adjNowMin >= openMin && adjNowMin <= closeMin) {
                        isOpenNow = true;
                    }
                    
                    if (isOpenNow && todaySched.breakStart && todaySched.breakEnd) {
                        const bStart = timeToMin(todaySched.breakStart);
                        let bEnd = timeToMin(todaySched.breakEnd);
                        if (bEnd < bStart) bEnd += 24 * 60;
                        const adjNowMinB = (bEnd >= 24 * 60 && nowMin < bStart) ? nowMin + 24 * 60 : nowMin;
                        if (adjNowMinB >= bStart && adjNowMinB < bEnd) {
                            isOpenNow = false; // On break
                        }
                    }
                    return isOpenNow;
                  };
                  const isOpen = isStoreCurrentlyOpen(form.schedules);
                  const days = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'];
                  const todaySched = form.schedules[days[new Date().getDay()]];
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', background: isOpen ? '#f0fdf4' : '#fef2f2', borderRadius: '10px', border: isOpen ? '1px solid #dcfce7' : '1px solid #fee2e2' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <p style={{ fontSize: '13px', color: isOpen ? '#166534' : '#991b1b', margin: 0 }}>
                          Hours: <span style={{ fontWeight: '700' }}>
                            {todaySched && todaySched.isOpen 
                              ? `${formatTime12h(todaySched.open)} — ${formatTime12h(todaySched.close)}` 
                              : 'Closed Today'}
                          </span>
                        </p>
                      </div>
